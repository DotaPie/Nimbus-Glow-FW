...
// main switch globals
int mainSwitchState = HIGH; // off
int previousMainSwitchState = HIGH; // off
...
void loadPreferences()
{
    ...
    sprintf(defaultSoftAP_ssid, "Nimbus Glow #%d", rng_id);
...
void update_LED_strip()
{
    ...
    // use no color if switch is in OFF position   
    ...
    {
        color = (current_CPT == ColorPickerType::COLOR_HUE) ? calculateColorHueFromPickerPosition(currentColorHueIndex) : calculateColorTemperatureFromPickerPosition(currentColorTemperatureIndex);
    }
...
void setupMainSwitch()
{
    ...
    mainSwitchState = digitalRead(MAIN_SWITCH_PIN);
    previousMainSwitchState = mainSwitchState;
    CONSOLE_CRLF("OK")
}
...
void checkMainSwitch()
{
    mainSwitchState = digitalRead(MAIN_SWITCH_PIN);

    if(mainSwitchState != previousMainSwitchState)
    {
        previousMainSwitchState = mainSwitchState;
        delay(10); // short delay to prevent debounce, shouldn't be noticable in loop, won't be called often
